...
const Decode = (location, command) => {

    console.log(Util.DecodeLocation(location));

};

const Parse = (songIds, command) => {
...
const Login = (username, password, command) => {
    ...
    Flow(function*(cb, u) {
        ...
        if(err && err != 'ERROR_USER_NOT_EXIST') {
            console.error(err);
            return;
        }
        ...
        if(user) {
            ...
            let err = yield Database.Users.update({
                username: username,
            }, {
                $set: {
            ...
            let err = yield Database.Users.insert({
                username: username,
                userId: json.data.user_id,
                nickname: json.data.nick_name,
                cookies: jar._jar.serializeSync(),
...
const Sync = (username, command) => {
    ...
    Flow(function*(cb, u) {
        ...
        const insertSongBase = (songId, isAlive, callback) => {
        ...
        const insertSongDetail = (songId, songDetail, callback) => {
...
const ListRecommends = (username, command) => {
...
module.exports = {
    Decode, Parse, Login, Sync, ListRecommends, ListSongs, Cache,
};
